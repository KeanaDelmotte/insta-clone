...
interface PostFullViewProps {
  ...
  >;
  loggedInUserSavedPosts: PostInterface[] | undefined;
  setLoggedInUserSavedPosts: React.Dispatch<
    SetStateAction<PostInterface[] | undefined>
  >;
}
...
const PostFullView: React.FC<PostFullViewProps> = ({
  ...
  useEffect(() => {
    fetchWithAuth(
      `http://localhost:3001/posts/${postId ? postId : post?.id}`,
      retrieveToken(),
      {
        method: 'GET',
      },
  ...
  return (
    <div className={cn('fullpost-page', { 'page-popup': popup })}>
      ...
      >
        {fullPost && (
          <img
            src={GetPhoto(fullPost?.photos[0].filename)}
            alt={fullPost?.description}
            className="full-post_media"
          />
        )}
            ...
            <div className="comment-section_comments">
              {fullPost?.comments.map((comment) => (
                <Comment
                  ...
                  id={comment.id}
                  userimgfilename={GetProfileImage(comment.user)}
                  likes={comment.likes}
